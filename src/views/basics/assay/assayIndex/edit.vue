<template>
  <div>
    ...
    <el-dialog :append-to-body="true" title="修改运输公司" :visible.sync="dialogShow" width="600px" @close="cancelEdit">
      <el-form ref="editForm" :rules="rules" :inline="true" :model="form" label-width="120px">
        <el-form-item label="选择物资" prop="name">
          <goods-select :goods-id.sync="form.goodsId" :goods-name="form.goodsName" select-style="width: 400px" />
        </el-form-item>
...
<script>
import { updateAssayIndex } from '@/api/basics/assay/assayIndex'
  ...
  methods: {
    ...
    doEdit() {
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          updateAssayIndex(this.form).then(() => {
            this.$refs.editForm.resetFields()
